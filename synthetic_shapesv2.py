...
class Synthetic_Shape:
    def __init__(self, center, label, shape_source_dir="shape_images"):
        self.center = center
        self.label = label
    ...
    def draw_shape(self,img,center,textbox_background=None,textbox_border_thickness=None, fill=False, outline=False):
        ...
        if np.random.randint(2) or fill:
            img = cv2.fillPoly(img, [pts], textbox_background)
        if np.random.randint(2) or outline:
...
class Synthetic_Arrow:
    def __init__(self,thickness):
        ...
        self.height = random.randint(12-(3-self.thickness), 20-(9-self.thickness)) + self.thickness*2
        ...
        self.cnt = np.array(np.where(thresh < 200))
    ...
    def get_points(self):
        ...
        # get dims of shape
        maxes = self.cnt.max(axis=1)
        mins = self.cnt.min(axis=1)
        min_x, max_x, min_y, max_y = mins[0], maxes[0], mins[1], maxes[1]
        len_x = max_x - min_x
        len_y = max_y - min_y
